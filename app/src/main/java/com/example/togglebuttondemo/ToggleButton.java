...
public class ToggleButton extends View implements View.OnClickListener {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mCheckedRunnable == null) {
            mCheckedRunnable = new AnimateRunnable(this, 255,
                    0, (int) mInitOffset, (int) (width - mInitOffset - 2 * mCircleRadius));
        }
        if (mUnCheckedRunnable == null) {
            mUnCheckedRunnable = new AnimateRunnable(this, 0,
                    255, (int) (width - mInitOffset - 2 * mCircleRadius), (int) mInitOffset);
        }
    }
    ...
    public void animationChangeChecked(boolean checked) {
        ...
        post(() -> {
            ...
            if (checked) {
                postRunnable(mCheckedRunnable);
            } else {
                postRunnable(mUnCheckedRunnable);
            }
            if (mOnCheckedChangeListener != null) {
                mOnCheckedChangeListener.onCheckedChange(checked);
            }
    ...
    private static class AnimateRunnable implements Runnable {
        ...
        public AnimateRunnable(ToggleButton toggleButton, int currentAlpha, int targetAlpha, int currentOffset, int targetOffset) {
        ...
        public void run() {
            ...
            if ((toggleButton.mCurrentOffset < mTargetOffset && mOffsetStep > 0) || (toggleButton.mCurrentOffset > mTargetOffset && mOffsetStep < 0)) {
                toggleButton.postRunnable(this);
            } else {
